...
inline void DDS_writebits(unsigned int value, unsigned int bits) {
    ...
    if (DDS_bufsize + bits < 32) {
        ...
        if (DDS_cachepos + 4 > DDS_cachesize) if (DDS_cache == NULL) {
...
void DDS_deinterleave(unsigned char *data, unsigned int bytes, unsigned int skip, unsigned int block = 0,
...
void DDS_interleave(unsigned char *data, unsigned int bytes, unsigned int skip,
                    unsigned int block = 0) { DDS_deinterleave(data, bytes, skip, block, TRUE); }
...
void DDS_decode(unsigned char *chunk, unsigned int size,
    ...
    while ((cnt1 = DDS_readbits(DDS_RL)) != 0) {
        ...
        for (cnt2 = 0; cnt2 < cnt1; cnt2++) {
            ...
            if ((cnt & (DDS_BLOCKSIZE - 1)) == 0) if (ptr1 == NULL) {
    ...
    if (ptr1 != NULL) if ((ptr1 = (unsigned char *) realloc(ptr1, cnt)) == NULL) ERRORMSG();
...
unsigned char *readRAWfiled(FILE *file, unsigned int *bytes) {
    ...
    do {
        if (data == NULL) { if ((data = (unsigned char *) malloc(DDS_BLOCKSIZE)) == NULL) ERRORMSG(); }
        else if ((data = (unsigned char *) realloc(data, cnt + DDS_BLOCKSIZE)) == NULL) ERRORMSG();
...
void writeDDSfile(const char *filename, unsigned char *data, unsigned int bytes, unsigned int skip, unsigned int strip,
                  BOOLINT nofree) {
    int version = 1;
...
void writePNMimage(const char *filename, unsigned char *image, unsigned int width, unsigned int height,
                   unsigned int components, BOOLINT dds) {
    char str[DDS_MAXSTR];
...
unsigned char *readPNMimage(const char *filename, unsigned int *width, unsigned int *height, unsigned int *components) {
    ...
    if ((data = readDDSfile(filename, &bytes)) == NULL) if ((data = readRAWfile(filename, &bytes)) == NULL)
        return (NULL);
...
void writePVMvolume(const char *filename, unsigned char *volume,
    ...
    if (description == NULL && courtesy == NULL && parameter == NULL && comment == NULL) if (scalex == 1.0f &&
                                                                                             scaley == 1.0f &&
                                                                                             scalez == 1.0f)
        snprintf(str, DDS_MAXSTR, "PVM\n%d %d %d\n%d\n", width, height, depth, components);
    else
        snprintf(str, DDS_MAXSTR, "PVM2\n%d %d %d\n%g %g %g\n%d\n", width, height, depth, scalex, scaley, scalez,
                 components);
    else
    ...
    if (description == NULL && courtesy == NULL && parameter == NULL && comment == NULL) {
        ...
        if ((data = (unsigned char *) malloc(
                strlen(str) + width * height * depth * components + len1 + len2 + len3 + len4)) == NULL)
            ERRORMSG();
        ...
        writeDDSfile(filename, data, strlen(str) + width * height * depth * components + len1 + len2 + len3 + len4,
                     components, width);
    }
}
...
unsigned char *readPVMvolume(const char *filename,
    ...
    if ((volume = (unsigned char *) malloc((*width) * (*height) * (*depth) * numc + len1 + len2 + len3 + len4)) ==
        NULL)
        ERRORMSG();
    ...
    if (description != NULL) if (len1 > 1) *description = volume + (*width) * (*height) * (*depth) * numc;
    else *description = NULL;

    if (courtesy != NULL) if (len2 > 1) *courtesy = volume + (*width) * (*height) * (*depth) * numc + len1;
    else *courtesy = NULL;

    if (parameter != NULL) if (len3 > 1) *parameter = volume + (*width) * (*height) * (*depth) * numc + len1 + len2;
    else *parameter = NULL;

    if (comment != NULL) if (len4 > 1) *comment = volume + (*width) * (*height) * (*depth) * numc + len1 + len2 + len3;
    else *comment = NULL;
...
inline int DDS_get(unsigned short int *data,
                   ...
                   unsigned int i, unsigned int j, unsigned int k) { return (data[i + (j + k * height) * width]); }
...
inline double DDS_getgrad(unsigned short int *data,
    ...
    if (i > 0) if (i < width - 1)
        gx = (DDS_get(data, width, height, depth, i + 1, j, k) - DDS_get(data, width, height, depth, i - 1, j, k)) /
             2.0;
    ...
    if (j > 0) if (j < height - 1)
        gy = (DDS_get(data, width, height, depth, i, j + 1, k) - DDS_get(data, width, height, depth, i, j - 1, k)) /
             2.0;
    ...
    if (k > 0) if (k < depth - 1)
        gz = (DDS_get(data, width, height, depth, i, j, k + 1) - DDS_get(data, width, height, depth, i, j, k - 1)) /
             2.0;
...
unsigned char *quantize(unsigned char *data,
    ...
    if (linear)
        ...
        for (k = 0; k < depth; k++)
            for (j = 0; j < height; j++)
                for (i = 0; i < width; i++)
                    err[DDS_get(data3, width, height, depth, i, j, k)] += sqrt(
                            DDS_getgrad(data3, width, height, depth, i, j, k));
